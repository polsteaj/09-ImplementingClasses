...
class Point(object):
    ...
    def move_to(self, x, y):
        self.dist = self.dist + math.sqrt((x - self.x) ** 2 + (y - self.y) ** 2)
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        self.dist = self.dist + math.sqrt(dx ** 2 + dy ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def closer_to(self, p2, p3):
        dist1 = math.sqrt((p2.x - self.x)**2 + (p2.y - self.y)**2)
        dist2 = math.sqrt((p3.x - self.x)**2 + (p3.y - self.y)**2)
        if dist1 <= dist2:
            return p2
        return p3
